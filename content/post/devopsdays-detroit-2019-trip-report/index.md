...
Ramin also mentioned [Conway's Law](https://en.wikipedia.org/wiki/Conway%27s_law) and how folks ship their organizational charts as code more often than not. Conway's Law should be considered when selling OpenShift to customers. Folks might want to take some time to figure out how a customer's org chart gets placed in OpenShift in a logical way. It'd be an interesting thought exercise to come up with use cases and how different teams would arrive at OpenShift with the team's org chart as the lense it's seen through.
